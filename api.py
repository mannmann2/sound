...
def current():
    ...
    try:
        js = make_request(url, params['username'])
        # if es.exists("users", '_doc', params['username']):
        # else:
            # username = es.search("users", '_doc', q="display_name:"+params['username'])['hits']['hits'][0]['_id']
...
def trending():
    ...
    Q1 = ' OR '.join(get_friends(params['username']))
    Q = {
        "query": {
            "constant_score": {
                "filter": {"match": {"played_by": Q1}}
            }
        },
        "size": 0,
        "aggs": {
    ...
    agg = es.search('recent', body=Q)['aggregations']['artists']['buckets']
    return str(agg)
